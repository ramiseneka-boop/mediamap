...
export default function InstagramCatalogPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const rows: Pablik[] = (media||[]).map((r:any)=>({ id:r.id, username:r.username||'', subscribers:r.subscribers||0, cost_post:r.cost_post||0, sell_post:r.sell_post||0, cityName:r.city?.name||'', catName:r.category?.name||'', catIcon:r.category?.icon||'', avatarUrl:`https://unavatar.io/instagram/${r.username||''}` }))
